...
function Study({ study, pick }) {
    return (
        <tr className="study z-depth-1">
            ...
            <td><Modal type={(study.e) ? "done" : "clear"} text={study.eligibility_criteria} title="Eligibility Criteria" study_id={`${study.study_id}e`} pick={pick} /></td>
            <td><Modal type={(study.b) ? "done" : "clear"} text={study.brief_summary} title="Brief Summary" study_id={`${study.study_id}b`} pick={pick} /></td>
